...
La scrittura <<se $p$ allora $q$>> si traduce quindi con la scrittura $p\Rightarrow q$, che si legge ``$p$ implica $q$''.
...
\paragraph{La doppia implicazione} o \emph{equivalenza logica} di due proposizioni $p$ e $q$ dà luogo a una proposizione che in simboli si rappresenta $p\Leftrightarrow q$ (leggasi ``$p$ se e solo se $q$'') che è vera solo se $p$ e $q$ sono entrambe vere o entrambe false. La tavola di verità è la seguente:
\begin{center}
...
Le figure a lato hanno la stessa forma ma una è più grande dell'altra, la seconda infatti è stata ottenuta dalla prima raddoppiando i lati: in geometria si dicono \emph{simili}.

Queste due altre figure non hanno la stessa forma, non si somigliano affatto, però le loro superfici hanno la stessa estensione, in quanto sono costituite dallo stesso numero di quadratini: in geometria si dicono \emph{equivalenti}.

Le figure a lato hanno la stessa forma e le stesse dimensioni ma sono in posizioni differenti, è possibile spostarle una sull'altra e farle coincidere. Usualmente le chiamiamo figure uguali, in geometria si dicono \emph{congruenti}.

Le due figure a lato hanno la stessa forma e le stesse dimensioni, per rendersene conto basta ruotare per esempio la seconda figura in senso antiorario e poi trascinarla sulla prima per sovrapporla. Anche queste figure sono dette uguali nel linguaggio comune, in geometria si dicono congruenti.

Le due figure a lato hanno stessa forma e stesse dimensioni, tuttavia non si riesce a trasportare l'una sull'altra muovendole nel piano, né trascinandole, né ruotandole, occorre ribaltarne una facendola uscire dal piano; le due figure sono una l'immagine speculare dell'altra. In geometria diciamo che sono \emph{inversamente congruenti}.
